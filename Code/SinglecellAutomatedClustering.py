...
def computing_and_embedding_neighbourhood_graphs(adata, numberOf_neighbors, numberOf_pcs, images_dir):
    ...
    sc.tl.umap(adata)
    # Get the number of principal components and neighbors as a string
    title_str = f"PCA-{numberOf_pcs}_Neighbors-{numberOf_neighbors}"
    
    # Plot UMAP with the custom title
    sc.pl.umap(adata, color=['CST3', 'NKG7', 'PPBP'], use_raw=False, title=title_str)
...
def main():
    ...
    num_neighbours = int(config['NeighbourhoodGraphs']['num_neighbours'])
