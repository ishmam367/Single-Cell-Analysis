...
def single_cell_analysis(adata,dataset_id,num_neighbors_values, num_pcs_values, clustering_algorithm, images_dir):
    ...
    for i, num_pcs in enumerate(num_pcs_values):
        for j, num_neighbors in enumerate(num_neighbors_values):
            ...
            # Dynamically call the clustering algorithm based on the provided name
            clustering_func = getattr(sc.tl, clustering_algorithm)
            clustering_func(adata_copy)
...
def main():
    ...
    clustering_algorithm = config['Parameters']['clustering_algorithm']

    adata = single_cell_analysis(adata, dataset_id, num_neighbors_values, num_pcs_values,clustering_algorithm, images_dir=BASE_IMG_DIR)
